...
2. 在对话框顶部选择类型（链接附件 / 笔记），按需添加条件行（字段范围 × 包含/不包含 × 值），列表随条件实时过滤；默认不勾选任何行，手动勾选或点 `全选`。
...
2. 插件把同一父条目下文件名相同（忽略大小写）的文件附件（含带来源网址的存储文档，如 PDF 全文）或 URL 相同的链接附件归为一组，在对话框中按组列出，每组默认保留一份：优先保留带批注的（批注多的优先），否则保留最早添加的；其余默认勾选为待删除。
